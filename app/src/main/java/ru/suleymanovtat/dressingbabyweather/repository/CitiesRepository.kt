...
class CitiesRepository(val serverCommunicator: ServerCommunicator, val database: AppDatabase) {
    ...
    suspend fun getCities() = database.cityDao().getCity().map { it.mapToLocal() }
    ...
    suspend fun getWeather() {
        ...
        settings.lat.let {
            ...
            val weatherBase = serverCommunicator.getWeather(settings.lat!!, settings.lng!!, BuildConfig.API_KEY)
            weatherBase.let {
